...
}; // Every HTML element has a matching HTMLxxxElement type — <input> → HTMLInputElement, <form> → HTMLFormElement, <dialog> → HTMLDialogElement. The .close() method is the giveaway that it's specifically a dialog

export default function CreateEventModal({ refreshForNewEvent, modalRef }: CreateEventModalProps) {
  const [actionData, submitAction, isPending] = useActionState(createEventAction, undefined);
  ...
  useEffect(() => {
    ...
    if ('message' in actionData && actionData.message !== 'Event cancelled') {
      refreshForNewEvent();
    }
